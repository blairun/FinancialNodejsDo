const dayjs = require('dayjs')
const date1 = dayjs().format('YYYY-MM-DD')
const date2 = dayjs().subtract(1, 'months').format('YYYY-MM-DD')
const date3 = dayjs().subtract(2, 'months').format('YYYY-MM-DD')
const date4 = dayjs().subtract(3, 'months').format('YYYY-MM-DD')
const retrievalDate = dayjs().format()

module.exports = [
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 1,
    TransactionDate: date1,
    Description: 'Transaction 1 Pending',
    Amount: -100,
    Category: 'Shops,Hardware Store',
    Pending: 1,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 2,
    TransactionDate: date1,
    Description: 'Transaction 2 Positive',
    Amount: 200,
    Category: 'Credit',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID2',
    TransactionID: 3,
    TransactionDate: date1,
    Description: 'Transaction 3 Negative',
    Amount: -100,
    Category: 'Other',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  ///////////////////////////////////////////////
  ///////////////////////////////////////////////
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 4,
    TransactionDate: date2,
    Description: 'Transaction 4 Negative',
    Amount: -300,
    Category: 'Shops,Convenience Stores',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 5,
    TransactionDate: date2,
    Description: 'Transaction 5 Positive',
    Amount: 500,
    Category: 'Credit',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID2',
    TransactionID: 6,
    TransactionDate: date2,
    Description: 'Transaction 6 Negative',
    Amount: -50,
    Category: 'Service,Entertainment',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  ///////////////////////////////////////////////
  ///////////////////////////////////////////////
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 7,
    TransactionDate: date3,
    Description: 'Transaction 7 Negative',
    Amount: -55,
    Category: 'Shops,Hardware Store',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 8,
    TransactionDate: date3,
    Description: 'Transaction 8 Positive',
    Amount: 390,
    Category: 'Credit',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID2',
    TransactionID: 9,
    TransactionDate: date3,
    Description: 'Transaction 9 Negative',
    Amount: -250,
    Category: 'Service,Automotive,Maintenance and Repair',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  ///////////////////////////////////////////////
  ///////////////////////////////////////////////
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 10,
    TransactionDate: date4,
    Description: 'Transaction 10 Negative',
    Amount: -99,
    Category: 'Shops,Hardware Store',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID1',
    TransactionID: 11,
    TransactionDate: date4,
    Description: 'Transaction 11 Positive',
    Amount: 511,
    Category: 'Credit',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
  {
    UserID: 1,
    AccountID: 'AccountID2',
    TransactionID: 12,
    TransactionDate: date4,
    Description: 'Transaction 12 Negative',
    Amount: -75,
    Category: 'Service,Entertainment',
    Pending: 0,
    RetrievalDate: retrievalDate,
  },
]
